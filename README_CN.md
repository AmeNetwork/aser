...
Aser 配备了标准化的 AI 能力中间件，如记忆、RAG、MCP、CoT、API 和社交客户端。通过动态集成工具包，帮助开发者快速构建和部署 AI Agent。
...
请参考 `.env.example` 文件，并根据你的设置创建 `.env` 文件。无需配置所有环境变量，只需选择你需要的部分。
...
## 快速开始
如果你是克隆项目源码，在运行示例前请在根目录执行 `pip install -e .`，这样 Python 能从本地源码导入 aser 模块。如果通过 `pip install aser` 安装，可直接运行示例。
...
创建带知识库的 AI Agent [示例](./examples/agent_knowledge.py)
...
创建带链式思维的 AI Agent [示例](./examples/agent_cot.py)
...
多 Agent 路由 [示例](./examples/router_multi_agent.py)

多 Agent 顺序执行 [示例](./examples/sequential_multi_agent.py)

多 Agent 并行执行 [示例](./examples/parallel_multi_agent.py)

多 Agent 响应式执行 [示例](./examples/reactive_multi_agent.py)

多 Agent 分层执行 [示例](./examples/hierarchical_multi_agent.py)
